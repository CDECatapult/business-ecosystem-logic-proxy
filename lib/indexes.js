...
function innerSearchIndex(indexName, method, cb) {
  var si = indexDataStores[indexName];

  if (!si) {
    cb("There is not a search index for the given path");
  } else {
    method(si, cb);
  }
}

function safeIndexExecute(path, method) {
  return new Promise((resolve, reject) => {
    lock.acquire("searchIndex", function() {
      innerSearchIndex(path, method, (err, extra) => {
        if (err) {
          reject(err);
        } else {
          resolve(extra);
...
function queryIndexExecute(path, method) {
  return new Promise((resolve, reject) => {
...
/* eslint no-console: 0 */
function saveIndex(path, batchdata, opt) {
  console.log(`Saving index ${path}...`);
  return safeIndexExecute(path, function(si, cb) {
    ...
    dataStream
      ...
      .pipe(si.add())
      .on("data", () => {})
      .on("end", () => { console.log('Saved index', path); cb(null)})
      .on("error", err => cb(err));
  });
}
...
function query(path, query, isCount) {
...
function getUserFromOffer(id, def) {
  if (def) {
  ...
  return exports.searchProductId(id).then(results => {
    ...
    return Promise.reject(new Error(`The product ${id} is not indexed, result.length = ${results.length}`));
  });
}
...
function convertInventoryData(data) {
  var lastUpdate;
  ...
  return indexCreateData(
    ["href", "name", "status", "startDate", "orderDate", "terminationDate"],
    data,
    initialData
  );
}

var addOfferInformationToInventory = function addOfferInformationToInventory(
  data
) {
  return exports.searchOfferingId(data.productOffering.id).then(results => {
    ...
    return Promise.reject(
      "The offering specified in the product is not indexed"
    );
  });
};
...
var convertProductData = function convertProductData(data) {
  ...
  return indexCreateData(
    ["name", "href", "lifecycleStatus", "isBundle", "productNumber"],
    data,
    initialData
  );
};
...
var createUrl = function createUrl(api, extra) {
  return (
    utils.getAPIProtocol(api) +
    "://" +
    utils.getAPIHost(api) +
    ":" +
    utils.getAPIPort(api) +
    "/" +
    api +
    extra
  );
    ...
    return blockPromises(data.category, cat => {
      return new Promise((resolve, reject) => {
        var url = createUrl(
          ...
          "/api/catalogManagement/v2/category/" + cat.id
        );

        request(url, function(err, response, body) {
          if (err || response.statusCode >= 400) {
            reject("One of the categories could not be indexed");
          } else {
      ...
      initialData.categoriesName = categories.map(x =>
        md5(x.name.toLowerCase())
      );
      return indexCreateData(
        ["href", "name", "lifecycleStatus", "isBundle"],
        data,
        initialData
      );
    });
...
function convertNotBundle(data, user) {
  return getUserFromOffer(data.productSpecification.id, user).then(user =>
    convertOffer(data, user)
  );
}

function convertBundle(data, user) {
  if (user) {
    ...
    return exports
      ...
      .then(offer =>
        getUserFromOffer(offer[0].document.productSpecification, user)
      )
      .then(user => convertOffer(data, user));
  }
}

function convertOfferingData(data, user) {
  if (!data.isBundle) {
...
function blockConvert(data, user) {
...
var convertOrderData = function convertOrderData(data) {
  ...
  return indexCreateData(
    ["href", "priority", "category", "state", "notificationContact", "note"],
    data,
    initialData
  );
};
...
exports.getMiddleware = function getMiddleware(reg, createOfferF, queryF, req) {
  ...
  if (
    ...
    if (
      q.query.length <= 0 ||
      (q.query.length == 1 && Object.keys(q.query[0].AND).length <= 0)
    ) {
      q.query = [{ AND: { "*": ["*"] } }];
    }
    ...
    if ("action" in req.query && req.query.action == "count") {
      ...
      handler = results => {
        var newUrl =
          ...
          "?id=" +
          results.map(r => r.document.originalId).join(",");
...
function product(arr) {
  return _product(arr, 0, arr.length);
}
...
var transformQuery = function transformQuery(query) {
  ...
  for (var qs of product(query.OR)) {
